...
int go(){
	...
	while (1){
		...
		if (SNES.V_Count > (SNES.NTSC ? 261 : 311)){
		...
		else if (SNES.V_Count < GFX.ScreenHeight){ 
			...
			(DMA_PORT[0x00]&0x20) && !(DMA_PORT[0x00]&0x10) &&
			SNES.V_Count == (DMA_PORT[0x09]&0xff)+((DMA_PORT[0x0A]&0xff)<<8)){
				GoIRQ();
		}
		else if (SNES.V_Count == (SNES.NTSC ? 261 : 311)){
			//V_blank
		...
		else if (SNES.V_Count == GFX.ScreenHeight || SNES.DelayedNMI){
			if (DMA_PORT[0x00]&0x80){
				GoNMI();
			}
			SNES.HIRQ_ok = 1;
			GFX.nb_frames++;
			if(GFX.nb_frames >= 10 && !GUI.log && !CFG.Debug){
				GFX.speed = GFX.nb_frames * 10 / (GFX.DSFrame - GFX.DSLastFrame);
