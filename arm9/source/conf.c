...
static void destroy_config(CONFIG *cfg)
{
   ...
   if (cfg) {
	 ...
	 if (prev->name)
	    TGDSARM9Free(prev->name);

	 if (prev->data)
	    TGDSARM9Free(prev->data);
...
static void set_config(CONFIG **config, char *data, int length, char *filename)
{
   ...
   *config = TGDSARM9Malloc(sizeof(CONFIG));
   ...
   if (filename) {
      (*config)->filename = TGDSARM9Malloc(strlen(filename)+1);
      if ((*config)->filename)
   ...
   while (pos < length) {
      ...
      if (in_section)
      {
	  p_e = TGDSARM9Malloc(sizeof(CONFIG_ENTRY));
          if (!p_e)
    	    return;

          if (name[0]) {
	     p_e->name = TGDSARM9Malloc(strlen(name)+1);
	     if (p_e->name)
	  ...
	  p_e->data = TGDSARM9Malloc(strlen(val)+1);
	  if (p_e->data)
	     strcpy(p_e->data, val);
	  ...
	  *prev_e = p_e;
      ...
      p = TGDSARM9Malloc(sizeof(CONFIG_SECTION));
      if (!p)
	 return;
      ...
      if (name[0]) {
	 p->name = TGDSARM9Malloc(strlen(name)+1);
	 if (p->name)
	    strcpy(p->name, name);
      }
      else
	 p->name = NULL;
      ...
      p->data = TGDSARM9Malloc(strlen(val)+1);
      if (p->data)
	 strcpy(p->data, val);
...
void hook_config_section(char *section, int (*intgetter)(char *, int), char *(*stringgetter)(char *, char *), void (*stringsetter)(char *,char *))
{
   ...
   hook->section = TGDSARM9Malloc(strlen(section_name)+1);
   if (!(hook->section)) {
      TGDSARM9Free(hook);
      return;
   }
...
static CONFIG_ENTRY *insert_variable(CONFIG *the_config, CONFIG_ENTRY *p, char *name, char *data)
{
   ...
   if (name) {
      n->name = TGDSARM9Malloc(strlen(name)+1);
      if (n->name)
	 strcpy(n->name, name);
   }
   else
   ...
   if (data) {
      n->data = TGDSARM9Malloc(strlen(data)+1);
      if (n->data)
	 strcpy(n->data, data);
   }
   else
