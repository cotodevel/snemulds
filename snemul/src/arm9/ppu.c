...
//  GFX.tiles2b_def[tile_addr/16] = (tile_addr_base>>13)+1;        
...
void	draw_plane(int bg, int bg_mode, int nb_tilex, int nb_tiley, int tile_size)
{
  ...
  if (tile_size == 3)
  {
	...
	{
		memset(GFX.BG3TilePriority, 0, 32);
	}
  	...
  	} else if (nb_tilex == 64 && nb_tiley == 64)
...
/*  	if ((SNES.V_Count & 7) == 0)
  ...
  //SB = (PPU_PORT[0x2C]&~PPU_PORT[0x2D])&CFG.BG_Layer&0xF;
  //if (SB&0x08) order[3]=3;
  //if (SB&0x04) order[2]=3;
  //if ((SB&0x04) && !(PPU_PORT[0x05]&8)) order[2] = 2;
...
/*  if (SNES.V_Count == CFG.Debug2)
  ...
  if (CFG.TilePriorityBG != -1)
  {
  	...
  	l->lBG3_CR = BG_16_COLOR | order[3] | (GFX.map_slot_ds[3]<<8) |
  ...
  l->lBG0_CR = BG_16_COLOR | order[0] | (GFX.tile_slot[0]<<2)  | (GFX.map_slot_ds[0]<<8) | GFX.map_size[0];
  l->lBG1_CR = BG_16_COLOR | order[1] | (GFX.tile_slot[1]<<2)  | (GFX.map_slot_ds[1]<<8) | GFX.map_size[1];
  if (CFG.Scaled != 0 || (GFX.YScroll == 16 && CFG.BG3Squish == 0))
  l->lBG2_CR = BG_16_COLOR | order[2] | (GFX.tile_slot[2]<<2)  | (GFX.map_slot_ds[2]<<8) | GFX.map_size[2];
...
void PPU_RenderLineMode3(uint32 MODE_1, uint32 MODE_2, t_GFX_lineInfo *l)
{
  ...
  // FIXME: should block interrupt here
  l->lDISPLAY_CR = MODE_0_2D | DISPLAY_SPR_2D | (SB << 8);
  l->lBG0_CR = BG_256_COLOR | order[0] | (GFX.tile_slot[0]<<2)  | (GFX.map_slot_ds[0]<<8) | GFX.map_size[0];
  l->lBG1_CR = BG_16_COLOR | order[1] | (GFX.tile_slot[1]<<2)  | (GFX.map_slot_ds[1]<<8) | GFX.map_size[1];
...
void PPU_RenderLineMode7(t_GFX_lineInfo *l)
{
	...
	l->lDISPLAY_CR = MODE_2_2D | DISPLAY_SPR_2D | (SB << 8);
  	...
  	if (!SNES.Mode7Repeat)
	  	l->lBG3_CR = BG_256_COLOR | (GFX.tile_slot[0]<<2)| BG_RS_128x128 | BG_PRIORITY(3) | BG_WRAP_ON;
	else
		l->lBG3_CR = BG_256_COLOR | (GFX.tile_slot[0]<<2) | BG_RS_128x128 | BG_PRIORITY(3) ;
...
{ 
  ...
  {	
	...
	l->lBG2_CR = BG_16_COLOR | order | (GFX.tile_slot[2]<<2)  | (GFX.map_slot_ds[2]<<8) | GFX.map_size[2];
...
void draw_screen()
{
	...
	BRIGHTNESS = (2<<14) | ((0x0f - GFX.brightness)<<1);
